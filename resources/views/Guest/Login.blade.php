<x-guest_layout>
    ...
    <div class="flex flex-col justify-center items-center px-6 py-16 space-y-16">
        ...
        <!-- Flash Messages -->
        @if (session('error'))
            <div class="w-full max-w-lg bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative">
                {{ session('error') }}
            </div>
        @endif

        @if (session('success'))
            <div class="w-full max-w-lg bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative">
                {{ session('success') }}
            </div>
        @endif
